...
def _parse_form_orders():
    ...
    if not initService():
        return None, "Google Sheets 連線失敗"
    ...
    for row in raw[GoogleSpreadDataParser.interestColumn].iterrows():
        try:
            ...
            all_new.append({
                ...
                "mPhone": dp.mPhone or "",
                "numbers": dp.numbers,
                "arrivalDate": dp.arrivalDate or "",
                "shippingDate": dp.shippingDate or "",
                "arrivalTime": dp.arrivalTimeFormat,
                "paymentMethod": dp.paymentMethodFormat,
            })
        except Exception:
            continue
    return all_new, None
...
def api_all_orders():
    ...
    for dp in orders:
        ...
        d['submittedAt'] = ts.strftime('%Y/%m/%d %H:%M') if ts else ''
        all_orders.append(d)
